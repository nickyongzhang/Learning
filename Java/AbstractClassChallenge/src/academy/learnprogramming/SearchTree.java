...
public class SearchTree implements NodeList {
    ...
    public boolean addItem(ListItem newItem) {
        if (this.root == null) {
            // the tree is empty, so our item becomes the head of the tree
            this.root = newItem;
            return true;
        }
    ...
    private void performRemoval(ListItem item, ListItem parent) {
        ...
        if (item.next() == null) {
            // no right tree, so make parent point ot left tree (which may be null)
            if (parent.next() == item) {
            ...
            if (parent.next() == item) {
                ...
                // again, we are deleting the node
                this.root = item.next();
            }
        } else {
